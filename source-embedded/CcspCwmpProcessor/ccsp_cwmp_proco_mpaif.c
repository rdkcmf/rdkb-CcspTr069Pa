    ...
    BOOL bFreeMemory)
{
    ...
    /* No need to map if the name is invisible */
    if (*pbIncludeInvQuery ||
        !CcspTr069PA_IsNamespaceInvisible(hTr069PaMapper, *pParamName))
    {
    ...
    )
{
    ...
    for ( i = 0; i < pParamNameArray->VarCount; i++ )
    {
        ...
        // No need to map items which are invisible
        if (!*pbIncludeInvQuery && CcspTr069PA_IsNamespaceInvisible(hTr069PaMapper, pParamName))
        {
    ...
    *phSoapFault       = (ANSC_HANDLE)NULL;
    ...
    if ( TRUE )
    {
		...
		GetParamSuccessStatus = TRUE;
			...
			{
				strncpy(tmp,paramval,sizeof(tmp));
				tmp[sizeof(tmp) -1] = '\0';
			}
			else
			{
				...
				if(strlen(tmp))
					strcat(tmp,"_");
#endif
	                      strncat(tmp,pParamValues[k]->parameterValue,64);
			      tmp[64 -1] = '\0';
                      }
                      else
                      {
    ...
    )
{
    ...
    *phSoapFault     = (ANSC_HANDLE)NULL;
    ...
    if ( CcspTr069PaMapFirstInternalAlias( pCcspCwmpCpeController->hTr069PaMapper, &pObjName, &bIncludeInvQuery, FALSE) )
    {
